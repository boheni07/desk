...
| 버전 | V2.4 (수치 통일: 알림 중복 제거, Rate Limit 2계층 명시, API 응답 형식 통일) |
...
| `PROXY_APPROVAL_COMPLETED` | 대리 승인 완료 | 등록자 + 지원담당자 |
...
| **Rate Limiting** | **계층 1: 계정 잠금** — 로그인 5회 연속 실패 시 15분 잠금(BUSINESS_RULES). **계층 2: IP Rate Limit** — 로그인 IP당 10회/15분, Presigned URL 사용자당 20회/시간, 전체 API 사용자당 100회/분, Push 구독 사용자당 10회/일. Redis sliding window |
