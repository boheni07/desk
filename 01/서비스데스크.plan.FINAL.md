...
| 변경 근거 | 2차 전문가 6인 검토 + Bootstrap 5 전환 + 사용자 결정 반영 + 구현 완료 확정 |
...
- 비밀번호 초기화 (관리자 기능): 초기화 시 임시 비밀번호 생성 → 이메일 등록 시 발송, 미등록 시 관리자에게 임시 비밀번호 표시 → 첫 로그인 시 강제 변경
- **비활성화 처리:**
...
- **프로젝트코드:** 시스템 전체에서 유니크, 영문대문자+숫자+하이픈, 3~20자
...
| **보안** | 서버 세션 + Redis 인증, RBAC (API 단위 매트릭스 적용), HTTPS, 비밀번호 bcrypt, 세션 쿠키 HttpOnly/Secure/SameSite=Strict, XSS 방어(사용자 입력 escape), 로그인 이력 기록(IP/시각/성공여부), **role_hint 쿠키 HMAC 서명 적용**, **CSRF: SameSite + Origin 헤더 화이트리스트** |
| **가용성** | 99.5% 이상 (월간) |
