...
|---|----------|------|------|----------|
| 1 | 배치 잡 수 | 9개 | **10개** (login-history-cleanup 추가) | 분석설계: 개인정보보호법 |
| 2 | 오류 코드 | 13종 | **17종** (+PROJECT_ACCESS_DENIED, EXTEND_ALREADY_PROCESSED, FILE_TOO_LARGE, TICKET_CANCEL_NOT_ALLOWED) | 분석설계 |
| 3 | RBAC 매트릭스 | 47개 | **52개+** (이력 조회, 담당자 배정, 프로젝트 비활성화, DLQ 관리 API 추가) | 분석설계 |
| 4 | Rate Limiting | 미정의 | **4개 정책** (로그인/Presigned/전체API/Push구독) | 운영관리 |
| 5 | 비밀번호 변경 세션 | 미정의 | **전체 세션 폐기 + 재로그인** (OWASP) | 사용자 결정 |
| 6 | FR-28 온보딩 | 단계 선언만 | **풀 마법사 4단계 상세** (Modal, ProgressBar, DB 상태 저장) | 사용자 결정 |
| 7 | 트랜잭션 범위 | 7개 | **8개** (자동접수 배치 추가) | 분석설계 |
| 8 | DB 백업 | 미정의 | **백업 정책** (자동 일일 + pg_dump 주 1회) | 운영관리 |
| 9 | Graceful Shutdown | 미정의 | **SIGTERM 핸들링** (Worker.close + 30초 대기) | 운영관리 |
| 10 | UI 프레임워크 | shadcn/ui + Tailwind | **Bootstrap 5 + React-Bootstrap v2.x + SCSS** | 사용자 요구 |
| 11 | 알림 이벤트 | 19개 | **21개** (+EXTEND_AUTO_APPROVED, BATCH_JOB_FAILED) | Design 동기화 |
| 12 | Free Plan 제한 | 미정의 | **Free Plan 제한 상수 테이블** 추가 | 분석설계 |
| 13 | Health Check | 미정의 | **DB/Redis/Worker 상태 포함** | 운영관리 |
| 14 | SC 추가 | SC-01~12 | **SC-13 온보딩 완료율 70%** 추가 | 기획 |
...
| Free Plan 최대 사용자 | **3명** | [V2.1] agent 수 제한 확정 |
| Free Plan 최대 저장 용량 | **5GB** | [V2.1] 티켓+댓글 첨부파일 합산 |
| Free Plan 데이터 보존 | **1년** | [V2.1] Pro+ 5년 |
| Free Plan SLA 관리 | **미제공** | [V2.1] Pro부터 |
...
| POST /api/tickets/[id]/assign | ✅ | ✅* | ❌ | *Main 담당자만 |
...
| `EXTEND_AUTO_APPROVED` | 연기 자동 승인 완료 [V2.1] | 지원담당자 + 등록자 |
| `BATCH_JOB_FAILED` | 배치 잡 DLQ 최종 실패 [V2.1] | 전체 admin |
...
- **비밀번호 변경 시 세션 처리 [V2.1]:** 변경 성공 시 현재 세션 포함 전체 세션 즉시 폐기(Redis에서 해당 userId의 모든 session 키 삭제) → 로그인 페이지 리다이렉트. OWASP 권고 준수.
...
| **배치 스케줄러** | BullMQ 기반 **9개** 배치 잡 [V2.0: 7→9개]. 재시도 3회 **exponential backoff (delay: 2초, factor: 2)** [V2.0]. Startup recovery. **DLQ:** 3회 재시도 후 최종 실패 시 Dead Letter Queue에 적재, 관리자에게 알림 발송 [V2.0]. **Graceful Shutdown [V2.1]:** SIGTERM 수신 시 Worker.close() → 진행 중 잡 완료 대기(최대 30초) → 프로세스 종료. **DLQ 재처리 [V2.1]:** GET /api/admin/jobs (조회) + POST /api/admin/jobs/[jobId]/retry (재시도) API 제공 |
...
| **디자인 토큰** | 티켓 상태 9종 색상, 우선순위 4종 색상, 폰트(Pretendard), 공통 간격/반경 — **Bootstrap SCSS 변수 + CSS 커스텀 프로퍼티** [V2.1] |
| **Rate Limiting** | 로그인: IP당 10회/15분, Presigned URL: 사용자당 20회/시간, 전체 API: 사용자당 100회/분, Push 구독: 사용자당 10회/일. Redis sliding window [V2.1] |
| **Backup** | PostgreSQL: Railway/Fly.io 자동 일일 백업 + pg_dump 주 1회 R2 저장. Redis: RDB 스냅샷(managed). 복구 테스트 분기 1회 [V2.1] |
| **Health Check** | GET /api/health — DB ping + Redis ping + BullMQ Worker 상태 + 마지막 배치 실행 시각 반환 [V2.1] |
...
| `PROJECT_ACCESS_DENIED` | 403 | 미배정 프로젝트 접근 [V2.1] |
| `EXTEND_ALREADY_PROCESSED` | 409 | 이미 승인/반려된 연기요청 재처리 [V2.1] |
| `FILE_TOO_LARGE` | 413 | 파일 크기 초과 (10MB/파일, 50MB/티켓) [V2.1] |
| `TICKET_CANCEL_NOT_ALLOWED` | 422 | REGISTERED 외 상태에서 취소 시도 [V2.1] |
...
| 8 | 자동접수 배치 [V2.1] | Ticket UPDATE(RECEIVED) + DeadlineHistory INSERT + 알림 INSERT | **건별 독립 트랜잭션** (실패 건 DLQ) |
...
| 10 | `login-history-cleanup` | **매일 03:30** [V2.1 추가] | 1년 초과 로그인 이력 물리 삭제. LIMIT 10000건/회 | exponential 2s |
...
| 레이어 | 기술 |
|--------|------|
| **Frontend** | Next.js 15 (App Router), React 19, TypeScript, **Bootstrap 5, React-Bootstrap v2.x, SCSS** [V2.1], Pretendard 폰트 |
...
| SC-13 | 온보딩 4단계 완료율 [V2.1] | Analytics 이벤트 로그 | 70% |
