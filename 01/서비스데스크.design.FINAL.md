...
| 작성일 | 2026-04-09 |
| 버전 | Final (Consolidated from V1.1~V2.1) |
...
│   ├── constants.ts
│   └── __tests__/
│       ├── business-hours.test.ts         # 81개 테스트
│       └── ticket-state-machine.test.ts   # 53개 테스트
│
...
| 4 | satisfaction-close | 1시간 | exp 2s | ✅ | **reminderSentAt** + 완료요청 3회 자동승인 통합 |
...
> **[구현 확인]** `autoApproveComplete3rd`는 독립 함수 대신 `satisfaction-close.job.ts`에 통합 구현됨. 기능적으로 동등하며, 코드 응집도 측면에서 유리하다.
...
- 불일치 시 403 응답 (CSRF_REJECTED)

### 7.3 보안 헤더 (next.config.ts) — QA에서 추가

```typescript
// next.config.ts — headers() 함수
